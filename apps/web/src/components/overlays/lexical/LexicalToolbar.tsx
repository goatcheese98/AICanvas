...
export function LexicalToolbar({
	...
	return (
		...
		>
			...
			<button
				type="button"
				style={{ ...buttonStyle(isBold), fontWeight: 700, fontFamily: 'serif', fontSize: 14 }}
				onMouseDown={(event) => event.preventDefault()}
			...
			<button
				type="button"
				style={{ ...buttonStyle(isUnderline), textDecoration: 'underline', fontSize: 14 }}
				onMouseDown={(event) => event.preventDefault()}
			...
			<button
				type="button"
				style={{ ...buttonStyle(isStrikethrough), textDecoration: 'line-through', fontSize: 14 }}
				onMouseDown={(event) => event.preventDefault()}
			...
			<button
				type="button"
				style={{ ...buttonStyle(false), fontSize: 11, letterSpacing: '-0.01em' }}
				onMouseDown={(event) => event.preventDefault()}
			...
			<button
				...
				type="button"
				style={{
					...buttonStyle(openDropdown === 'text-color'),
			...
			<button
				...
				type="button"
				style={{
					...buttonStyle(openDropdown === 'highlight'),
					flexDirection: 'column',
					gap: 2,
					padding: '3px 5px',
					height: 28,
			...
			>
				...
				<span
					style={{
						width: 14,
						height: 2.5,
						background: highlightColor || '#d6d3d1',
						borderRadius: 1,
					}}
				/>
			</button>
			...
			<button
				ref={tableRef}
				type="button"
				style={buttonStyle(openDropdown === 'table')}
				onMouseDown={(event) => event.preventDefault()}
							...
							<input
								...
								onChange={(event) =>
									label === 'Rows'
										...
										: setTableCols(event.target.value)
								}
								style={{
									width: 56,
									padding: '4px 6px',
			...
			<button
				ref={equationRef}
				type="button"
				style={buttonStyle(openDropdown === 'equation')}
				onMouseDown={(event) => event.preventDefault()}
				...
				<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
					...
					>
						<input
							...
							onChange={(event) => setEquationInline(event.target.checked)}
						/>
						Inline
					</label>
					<button
			...
			{onToggleWordCount ? (
				<button
					type="button"
					style={buttonStyle(Boolean(showWordCount))}
					onMouseDown={(event) => event.preventDefault()}
