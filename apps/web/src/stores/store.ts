...
import { createChatSlice, type ChatSlice, type ChatThread } from './slices/chatSlice';
...
function coercePersistedThreads(state: Partial<AppStore>, fallbackThreads: ChatThread[]) {
	...
	if (Array.isArray(state.messages) && state.messages.length > 0) {
		return [
			{
				...fallbackThreads[0],
				title: state.messages[0]?.role === 'user' ? state.messages[0].content.slice(0, 40) : 'Recovered chat',
