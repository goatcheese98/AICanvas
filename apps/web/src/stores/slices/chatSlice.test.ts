...
describe('chatSlice', () => {
    ...
    describe('addMessage', () => {
        ...
        it('updates thread title from first user message', () => {
            ...
            store.getState().addMessage(message);
            expect(store.getState().chatThreads[0].title).toBe('This is a long message that should be tr');
        });
        ...
        it('does not update title if not first user message', () => {
            ...
            store.getState().addMessage(createMessage({ content: 'Second message with different content' }));
            expect(store.getState().chatThreads[0].title).toBe('First message');
        });

        it('does not update title from assistant message', () => {
            store.getState().addMessage(createMessage({ role: 'assistant', content: 'Assistant response' }));
            expect(store.getState().chatThreads[0].title).toBe('New chat');
        });
    });
    ...
    describe('setActiveChatThread', () => {
        ...
        it('clears chat error when switching threads', () => {
            ...
            const otherThreadId = store.getState().chatThreads.find(
                (t) => t.id !== store.getState().activeChatThreadId,
            )!.id;
...
describe('chatSlice helper functions', () => {
    describe('summarizeThreadTitle', () => {
        ...
        it('truncates long content to 40 characters', () => {
            const longContent = 'This is a very long message that should be truncated to forty characters';
        ...
        it('normalizes whitespace', () => {
            expect(summarizeThreadTitle('Message   with   extra   spaces')).toBe('Message with extra spaces');
        });
    });
});
