...
export function buildPrototypePrompt(userText: string, currentPrototypeJson?: string): string {
	return [
		...
		'- dependencies must be a JSON object map like { "lucide-react": "" }, not an array.',
		'- files must be a JSON object map where each key is an absolute path and each value is an object with at least a "code" string.',
